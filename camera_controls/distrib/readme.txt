    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_camera_controls.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: camera_controls.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * camera_controls.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * camera_controls.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * camera_controls (application created by MATLAB Compiler)
        * run_camera_controls.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * camera_controls.dll (shared library, file extension varies by 
          platform)
        * camera_controls.h (library header file)
    ...
    Windows:
        * camera_controls.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * camera_controls.xml (documentation files)
        * camera_controls.pdb (program database file - if DEBUG option 
          selected)
        * camera_controls.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * camera_controls.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * camera_controls_0_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * camera_controls.jar (Java class files and compiled M-code)
    ...
    Windows:
        * camera_controls.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * camera_controls_0_0.dll (COM component)
        * camera_controls.xla (optionally created and placed in
          C:\Documents and Settings\Administrator\My Documents\MATLAB\my code\final code for tracking max object from live video\camera_controls\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
